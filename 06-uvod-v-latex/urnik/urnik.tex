...
$$\text{število delovnih dni} \times \text{število ur na dan}$$
...
\begin{itemize}
    ...
    \item Predavanja posebne vrste (npr.\ iz računalništva) se morajo izvajati v ustrezni predavalnici.
...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S $c_i$ označimo število
kršitev omejitve $i$, z $w_i$ pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
$$K = \sum_{i} c_i \cdot w_i$$
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   Z zanko se sprehodimo po vektorju vseh predmetov. Predmet na $k$--tem koraku
   označimo s $P_k$.

   Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
   $j$--tem koraku označimo s $D_j$.

   Za vse proste termine $T$ izračunamo ceno vstavljanja
   $C(T)$, ki ga sestavlja srečanje $M$, profesor, ki ta predmet predava,
   ...
   vstavljanja, da bomo srečanje $M$ na koncu vstavili v termin z najboljšo
   ceno vstavljanja.

   Ko preverimo vse proste termine vseh predavalnic, vstavimo srečanje $M$
...
Rešitev se ponuja kar sama -- za naslednjih $T$ korakov prepovemo vsako spremembo, ki jo
opravimo v koraku optimizacije. Izbira konstante $T$ je odvisna od samega problema, saj
ne obstaja neki splošen $T$, ki bi nas v vsakem primeru pripeljal iz lokalnega ekstrema.
...
Naj bo $T=5$ ter $S_1$ sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
spremembo (npr.\ zamenjava *$E_1$ in $E_2$) sedaj shranimo v
...
tem poskrbimo, da se lahko v isti lokalni maksimum vrnemo šele čez $T$ korakov, saj
...
\end{verbatim}
   ...
   se zaključi, če po $T$ korakih še vedno tičimo v istem lokalnem maksimumu.
%
